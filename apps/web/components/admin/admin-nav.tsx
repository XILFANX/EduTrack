...
export function AdminNav({ isRoot }: { isRoot: boolean }) {
  ...
  const NAV_ITEMS = [
    { label: 'Platform', href: '/admin/dashboard', icon: LayoutDashboard },
    { label: 'Schools', href: '/admin/schools', icon: Building2 },
  ]

  if (isRoot) {
    NAV_ITEMS.push({ label: 'Admins', href: '/admin/admins', icon: ShieldCheck })
  }
      ...
      <div className="max-w-md mx-auto flex items-center justify-around p-2">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <div className={`relative flex items-center justify-center w-8 h-8 rounded-full mb-0.5 transition-all ${isActive ? 'bg-indigo-100 dark:bg-indigo-900/40' : ''}`}>

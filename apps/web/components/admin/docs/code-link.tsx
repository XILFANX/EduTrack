...
export function CodeLink({ repoPath, accent = 'blue' }: CodeLinkProps) {
  ...
  const colorCls =
    accent === 'violet'
      ? 'border-violet-200 dark:border-violet-800/60 bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-300 hover:bg-violet-100 dark:hover:bg-violet-900/40 hover:border-violet-400 dark:hover:border-violet-600'
  ...
  return (
    ...
    >
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-transform group-hover:scale-110 ${
        accent === 'violet' ? 'bg-violet-100 dark:bg-violet-900/40' : 'bg-blue-100 dark:bg-blue-900/40'
      }`}>

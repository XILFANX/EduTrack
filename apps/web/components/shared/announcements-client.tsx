...
export function AnnouncementsClient({ audienceOptions, defaultAudience, subjectPlaceholder = "e.g. Important Update" }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      {status && (
        <div className={`p-3 rounded-xl text-sm font-medium ${status.type === 'success' ? 'bg-blue-500/10 text-blue-500 dark:text-blue-400' : 'bg-orange-500/10 text-orange-500 dark:text-orange-400'}`}>
          {status.text}
        </div>
      )}

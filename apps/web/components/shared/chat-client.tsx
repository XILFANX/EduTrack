...
export interface DirectoryCategory {
  id: string
  label: string
  iconName: 'users' | 'briefcase' | 'shield' | 'graduation-cap' | 'home'
  roles: string[]
}

function maskAdminContact(c: Contact): Contact {
  if (c.role === 'Platform Admin') {
    return { ...c, name: 'EduTrack Support', role: 'Support' }
  }
  return c
}
...
export function ChatClient({
  ...
  // Visible categories: admins/platform see only their relevant categories
  const visibleCategories = directoryCategories.filter(cat => {
          ...
          {searchQuery.trim() && !selectedCategory ? (
            ...
            <div className="p-3 space-y-1">
              ...
              {visibleCategories.map(category => {
                ...
                if (category.iconName === 'graduation-cap') Icon = GraduationCap
                if (category.iconName === 'home') Icon = Home
                return (
                  <button
                    key={category.id}

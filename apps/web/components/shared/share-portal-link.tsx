  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      <button
        ...
        className={`flex items-center gap-1.5 text-xs font-medium transition-colors text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-400 ${className}`}
      >
      ...
      {isOpen && (
        <div className="absolute left-0 mt-2 w-48 rounded-xl bg-card border border-border shadow-lg overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="py-1 flex flex-col">
            ...
            >
              {copied ? <Check className="w-4 h-4 text-blue-600" /> : <Copy className="w-4 h-4 text-muted-foreground" />}

    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-24">
      ...
      <div className="relative max-w-md">
        ...
        <input 
          ...
          className="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/20 text-foreground focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all text-sm placeholder:text-muted-foreground"
        />
      </div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {filtered.map(cls => (
                ...
                >
                  <div className="w-11 h-11 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center shrink-0 group-hover:bg-cyan-500/20 group-hover:border-cyan-500/30 transition-colors">
                    <span className="text-sm font-bold text-cyan-400">
                      {cls.name.substring(0, 2).toUpperCase()}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-foreground text-sm truncate group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">{cls.name}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {cls.countLabel || 'Manage module'}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="hidden sm:block text-xs font-semibold px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 group-hover:bg-cyan-600 group-hover:text-white transition-colors">
                      Open
                    </span>

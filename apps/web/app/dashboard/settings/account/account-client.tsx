...
export default function AccountClient({ initialName, initialPhone }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
      {status && (
        <div className={`p-3 rounded-xl text-sm font-medium ${status.type === 'success' ? 'bg-cyan-500/10 text-cyan-600 dark:text-cyan-400' : 'bg-orange-500/10 text-orange-600 dark:text-orange-400'}`}>
          {status.text}
        </div>
      )}
      ...
      <div className="space-y-1.5">
        ...
        <input
          ...
          required
          className="w-full bg-background border border-border rounded-xl px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-cyan-500/50"
        />
      </div>

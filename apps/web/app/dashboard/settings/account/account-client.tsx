...
export default function AccountClient({ initialName, initialPhone }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
      {status && (
        <div className={`p-3 rounded-xl text-sm font-medium ${status.type === 'success' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'bg-red-500/10 text-red-600 dark:text-red-400'}`}>
          {status.text}
        </div>
      )}

...
export default async function SettingsPage() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-3xl mx-auto pb-32">
      ...
      {/* Quick info card */}
      <div className="bg-white dark:bg-[#060d1a] text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-[#1a2744] rounded-xl p-5 shadow-sm space-y-3 text-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={item.label} className="flex justify-between py-1 border-b border-slate-50 dark:border-[#1a2744]/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {SETTINGS_SECTIONS.map((section) => (
          <Link
            key={section.href}
            href={section.href}
            className="group bg-white dark:bg-[#060d1a] text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-[#1a2744] rounded-xl p-4 shadow-sm hover:shadow-md hover:border-slate-300 dark:hover:border-slate-700 transition-all flex items-start gap-3"
          >
            <span className="text-2xl">{section.icon}</span>
            <div>
              <p className="font-medium text-foreground group-hover:text-blue-700 dark:group-hover:text-blue-500 transition-colors">{section.label}</p>

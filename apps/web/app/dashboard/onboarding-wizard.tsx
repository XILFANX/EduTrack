...
export function OnboardingWizard({ totalStaff, totalClasses, totalSubjects, totalStudents }: OnboardingWizardProps) {
  ...
  return (
    <div className="rounded-2xl border border-blue-200 dark:border-blue-500/30 bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-900/10 dark:to-blue-900/5 shadow-sm overflow-hidden mb-7">
      {/* Header */}
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="flex flex-col gap-2 md:grid md:grid-cols-4 md:gap-3">
            {steps.map((step, idx) => (
              <div
                key={step.id}
                className="flex items-center gap-3 bg-white/70 dark:bg-[#060d1a]/40 rounded-xl border border-blue-100 dark:border-blue-500/10 px-3 py-2.5 md:flex-col md:items-start md:p-4"
              >
                {/* Step number badge */}
                ...
                <div className="hidden md:flex items-start justify-between mb-1 w-full">
                  ...
                  {step.completed && <CheckCircle2 className="w-5 h-5 text-blue-600" />}
                </div>
                ...
                {step.completed ? (
                  ...
                  <Link
                    href={step.href}
                    className="shrink-0 flex items-center gap-1 text-xs font-semibold bg-[#1D6FEB] hover:bg-[#1558C8] text-white px-2.5 py-1.5 rounded-lg transition-colors"
                  >
                ...
                {!step.completed && (
                  <Link
                    href={step.href}
                    className="hidden md:flex items-center justify-center gap-1.5 w-full mt-2 py-2 rounded-lg bg-[#1D6FEB] hover:bg-[#1558C8] text-white text-xs font-semibold transition-colors"
                  >

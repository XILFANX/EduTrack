...
const ROLE_COLORS: Record<string, { badge: string; btn: string; glow: string }> = {
  tenant:    { badge: 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300', btn: 'bg-violet-600 hover:bg-violet-700', glow: 'from-violet-500/20 to-slate-900' },
  caretaker: { badge: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300',    btn: 'bg-amber-500 hover:bg-amber-600',   glow: 'from-amber-500/20 to-slate-900' },
}

export default function InviteClient({
  token, inviteId, role, roleLabel,
  landlordId, landlordName, propertyName, unitName,
  ...
  async function handleSendOtp() {
    ...
    if (result?.error) {
      ...
      setViewState('otp')
      setSuccessMsg('An OTP has been sent to your phone.')
    }
  }
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md space-y-6 z-10">
        ...
        <div className="text-center space-y-3">
          ...
          <div>
            <p className="text-muted-foreground text-sm font-medium">EstateTrack</p>
            ...
            {viewState === 'register' && (
              <p className="text-muted-foreground text-sm mt-2 leading-relaxed">
                ...
                <strong className="text-foreground">{propertyName || 'EstateTrack'}</strong>
                {' '}as a{' '}
                <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold ${colors.badge}`}>{roleLabel.toLowerCase()}</span>
                {role === 'tenant' && unitName && (
                  <>
                    {' '}in unit <strong className="text-foreground">{unitName}</strong>
                  </>
                )}
                .
              </p>
          ...
          {(propertyName || unitName) && (
            <div className="flex items-center justify-center gap-2 flex-wrap">
              ...
              {unitName && (
                <span className="inline-flex items-center gap-1.5 bg-violet-50 dark:bg-violet-900/30 border border-violet-200 dark:border-violet-800/50 text-violet-700 dark:text-violet-300 text-xs font-medium px-3 py-1.5 rounded-full">
                  🚪 Unit {unitName}
                </span>
              )}
            </div>
        ...
        <div className="bg-card/80 backdrop-blur-xl rounded-2xl border border-border p-6 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {viewState === 'otp' && (
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="123456"
                  maxLength={6}
                  className="w-full bg-muted border border-border text-foreground placeholder-muted-foreground rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition"
                />
              </div>
            )}

            {/* Full Name & Phone (Only show in register/login) */}
            {viewState !== 'otp' && (
              <>
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className={`w-full bg-muted border border-border text-foreground placeholder-muted-foreground rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition ${viewState === 'login' ? 'opacity-70 cursor-not-allowed' : ''}`}
                  />
                </div>

                <div className="space-y-1.5">
                  <label htmlFor="inv-phone" className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                    Phone Number {registeredPhone ? <span className="text-red-400 normal-case font-normal">(required)</span> : <span className="text-muted-foreground normal-case font-normal">(optional)</span>}
                  </label>
                  <input
            ...
            <div className="space-y-1.5 relative">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-muted border border-border text-foreground placeholder-muted-foreground rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition pr-10"
                />
                <button
                  type="button"
            ...
            {/* Confirm Password (Hide on login) */}
            {viewState !== 'login' && (
              <div className="space-y-1.5 relative">
            ...
            >
              {loading ? 'Please wait…' : viewState === 'login' ? `Access ${roleLabel} Portal →` : viewState === 'otp' ? 'Reset Password & Log In →' : `Create Account →`}
            </button>
          </form>

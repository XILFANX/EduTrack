...
import { TransportNav } from '@/components/transport/transport-nav'
...
export default async function TransportLayout({ children }: { children: React.ReactNode }) {
  ...
  if (profile?.role !== 'transport_matron') redirect('/login')
      ...
      <header className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-300 flex items-center justify-center font-bold text-lg">
              {profile.full_name[0]}
            </div>
            <div>
              <p className="font-semibold text-sm text-foreground">{profile.full_name}</p>
              <p className="text-xs text-teal-600 dark:text-teal-400 font-medium">Transport Portal</p>
            </div>
          </div>

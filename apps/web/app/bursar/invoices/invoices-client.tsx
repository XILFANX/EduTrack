      ...
      {filteredInvoices.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl">
          <div className="w-16 h-16 rounded-2xl bg-cyan-100 dark:bg-cyan-900/40 mx-auto flex items-center justify-center mb-4">
            <Receipt className="w-8 h-8 text-cyan-600 dark:text-cyan-400" />
          </div>
                ...
                {filteredInvoices.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-6 py-4 text-cyan-600 dark:text-cyan-400 font-medium">{formatter.format(inv.amount_paid)}</td>
                      <td className="px-6 py-4 font-bold text-orange-600 dark:text-orange-400">{formatter.format(balance)}</td>
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium uppercase tracking-wider ${
                          inv.status === 'paid' ? 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400' :

      ...
      <DialogContent className="max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-0 overflow-hidden">
        ...
        {result ? (
          <div className="p-6 text-center space-y-4">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto ${result.matched > 0 ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-orange-100 dark:bg-orange-900/30'}`}>
              <CheckCircle2 className={`w-8 h-8 ${result.matched > 0 ? 'text-blue-600' : 'text-orange-600'}`} />
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button className="flex-1 bg-blue-600 hover:bg-blue-700" onClick={handleClose}>Done</Button>
            </div>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <button
              type="button"
              onClick={addRow}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 text-xs text-muted-foreground hover:border-blue-400 hover:text-blue-600 transition-all"
            >

      ...
      <DialogContent className="max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-0 overflow-hidden">
        ...
        {result ? (
          <div className="p-6 text-center space-y-4">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto ${result.matched > 0 ? 'bg-cyan-100 dark:bg-cyan-900/30' : 'bg-amber-100 dark:bg-amber-900/30'}`}>
              <CheckCircle2 className={`w-8 h-8 ${result.matched > 0 ? 'text-cyan-600' : 'text-amber-600'}`} />
            </div>
            ...
            {result.errors.map((e, i) => (
              <p key={i} className="text-sm text-red-600 bg-red-50 dark:bg-red-950/30 px-3 py-2 rounded-lg">{e}</p>
            ))}
            ...
            {rows.map((row, idx) => (
              <div key={row.id} className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 space-y-3 relative">
                ...
                {(row.paymentRail === 'cash' || row.paymentRail === 'cheque') && (
                  <div className="flex gap-1.5 items-start bg-amber-50 dark:bg-amber-900/20 rounded-lg p-2">

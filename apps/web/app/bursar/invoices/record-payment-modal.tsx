...
export function VerifyPaymentModal({ open, onClose }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="max-w-lg bg-white dark:bg-[#060d1a] border border-slate-200 dark:border-[#1a2744] rounded-2xl shadow-2xl p-0 overflow-hidden">
        <DialogHeader className="px-6 pt-5 pb-4 border-b border-slate-100 dark:border-[#1a2744]">
        ...
        {result ? (
          <div className="p-6 text-center space-y-4">
            ...
            {result.errors.map((e, i) => (
              <p key={i} className="text-sm text-red-600 bg-blue-50 dark:bg-blue-950/30 px-3 py-2 rounded-lg">{e}</p>
            ))}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            {rows.map((row, idx) => (
              <div key={row.id} className="border border-slate-200 dark:border-[#1a2744] rounded-xl p-4 space-y-3 relative">
                {rows.length > 1 && (
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    id={`raw-${row.id}`}
                    rows={2}
                    className="w-full bg-slate-50 dark:bg-[#0d1b2e] border border-slate-200 dark:border-[#1a2744] rounded-xl px-3 py-2 text-xs resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/40 font-mono"
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg border text-xs font-medium transition-all ${
                        row.paymentRail === r.value
                          ...
                          : 'border-slate-200 dark:border-[#1a2744] text-muted-foreground'
                      }`}
                    >
                      {r.label}
                    </button>
                ...
                {(row.paymentRail === 'cash' || row.paymentRail === 'cheque') && (
                  <div className="flex gap-1.5 items-start bg-blue-50 dark:bg-blue-900/20 rounded-lg p-2">
            ...
            <button
              type="button"
              onClick={addRow}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 border-dashed border-slate-200 dark:border-[#1a2744] text-xs text-muted-foreground hover:border-cyan-400 hover:text-blue-600 transition-all"
            >

  ...
  return (
    <div className="space-y-4 mb-8">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto text-xs bg-amber-500/10 border border-amber-500/20 text-amber-500 px-2 py-0.5 rounded-full font-semibold">
          {rows.length} open
        </span>
      </div>
      ...
      <div className="space-y-3">
        {rows.map((d) => {
          ...
          return (
            <div key={d.id} className="bg-card border border-amber-500/20 rounded-2xl p-5 space-y-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="text-xs bg-amber-500/10 border border-amber-500/20 text-amber-500 px-2 py-0.5 rounded-full font-semibold capitalize">
              ...
              <div className="flex flex-wrap gap-2">
                <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${
                  codeMatch ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'
                }`}>
                  Code: {codeMatch ? '✓' : '✗'}
                </span>
                <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${
                  amountMatch ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-amber-500/10 border-amber-500/20 text-amber-400'
                }`}>
              ...
              <div className="flex gap-3">
                <form action={async () => { 'use server'; await dismissDispute(d.id) }}>
                  <button
                    type="submit"
                    className="text-sm border border-border text-muted-foreground hover:text-foreground hover:border-rose-400 py-2 px-4 rounded-xl font-medium transition-colors"
                  >
                    Dismiss
                  </button>
                </form>
                <form action={async () => { 'use server'; await resolveDispute(d.id) }} className="flex-1">
                  <button
                    ...
                    type="submit"
                    className="w-full text-sm bg-emerald-600 hover:bg-emerald-700 text-white py-2 px-4 rounded-xl font-semibold transition-colors flex items-center justify-center gap-2"
                  >

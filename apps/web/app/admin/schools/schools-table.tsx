...
export function SchoolsTable({ initialData }: { initialData: SchoolData[] }) {
  ...
  return (
    <div className="bg-white dark:bg-[#060d1a]/80 border border-border rounded-3xl shadow-sm overflow-hidden flex flex-col">
      {/* ── Filters ── */}
      <div className="p-4 md:p-5 border-b border-border bg-slate-50/50 dark:bg-[#0d1b2e]/20 flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
        <div className="relative max-w-sm w-full">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[#060d1a] border border-slate-200 dark:border-[#1a2744] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium"
          />
        </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-all ${
                tierFilter === tier 
                  ...
                  : 'bg-white dark:bg-[#0d1b2e] text-slate-500 border border-slate-200 dark:border-[#1a2744] hover:bg-slate-50 dark:hover:bg-[#0d1b2e]/80'
              }`}
            >
              {tier}
            </button>
      ...
      {!filteredData || filteredData.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center px-4">
          <div className="w-16 h-16 bg-slate-100 dark:bg-[#0d1b2e] rounded-2xl flex items-center justify-center mb-4">
        ...
        <div className="divide-y divide-border/50">
          {filteredData.map((s) => (
            <Link
              ...
              key={s.id}
              className="grid grid-cols-1 md:grid-cols-12 gap-4 p-5 hover:bg-slate-50 dark:hover:bg-[#0d1b2e]/80 transition-colors group items-center cursor-pointer"
            >
              <div className="md:col-span-5 flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-bold text-foreground truncate transition-colors group-hover:text-blue-600 dark:group-hover:text-blue-500">
                      {s.name}
                    </p>
                  </div>
              ...
              <div className="md:col-span-3 flex items-center justify-between md:justify-center">
                ...
                <span className={`text-[10px] px-3 py-1.5 rounded-full font-black uppercase tracking-wider shadow-sm ${
                  ...
                  'bg-slate-100 text-slate-600 border border-slate-200 dark:bg-[#0d1b2e] dark:border-[#1a2744]'
                }`}>
                  {s.subscription_tier}
                </span>
              </div>
              ...
              <div className="md:col-span-2 flex items-center justify-between md:justify-end">
                ...
                <div className="flex flex-col items-end">
                  <span className="text-sm font-bold text-foreground group-hover:text-blue-600 dark:group-hover:text-blue-500 transition-colors">

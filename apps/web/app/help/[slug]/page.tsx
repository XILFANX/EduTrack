      ...
      <div className="flex xl:gap-16 lg:gap-10 pb-20">
        ...
        <article className="flex-1 min-w-0 max-w-3xl">
          ...
          <div className="mt-20 pt-8 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-sm text-slate-500 dark:text-slate-400">
            <p>Still need help? <Link href="/contact" className="text-cyan-600 dark:text-cyan-400 hover:underline font-medium">Contact Support</Link></p>
              ...
              <ul className="space-y-2.5">
                {headings.map((heading) => (
                  <li key={heading.id}>
                    <a 
                      href={`#${heading.id}`}
                      className="text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors line-clamp-2 leading-relaxed"
                    >
                      {heading.title}
                    </a>
                  </li>

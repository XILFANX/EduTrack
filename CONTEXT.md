...
2. Make the change.
3. Run the Tier 1 gate from Step 3 — build, lint, and the tests relevant to the change. Since a fast-path change is the entire mission, this single pass also satisfies Tier 2; there's no separate final gate to run afterward.
4. Report back in chat.

> **Note:** Even on the fast path, fix any doc line the change makes wrong (a documented default, endpoint behavior, or setup step) in the same pass, and only after step 3 is clean — a fix that leaves a doc wrong behind it isn't actually low-risk, and a doc written against code that doesn't build yet isn't actually done. If nothing documented is affected, just proceed.
...
1. **Restate the request** in your own words. If the restatement doesn't match what was asked, you misread it — reread it, don't proceed on a guess.
2. **Separate** what was explicitly stated from what you're assuming. Assumptions are fine; silent ones aren't.
3. **Classify the mission**: bug fix, new feature, refactor, migration, investigation, documentation bootstrap, or review-only — and whether it qualifies for the Step 0 fast path. This decision shapes everything after it.
4. **Decide** whether this is fresh work or touches existing work. If it touches existing work, Step 2 isn't optional — you need to see what's actually there before proposing changes.
5. **Surface ambiguity now.** If the request is ambiguous in a way that would change the implementation (not just a style preference), ask now — not three steps later where no one reads it until review.
...
**Tier 1 gate:** build + lint + tests relevant to this task — must be clean
  *before* the docs line below is touched. See [Verification Tiers](#verification-tiers).
**Commit:** one atomic commit on completion (code + docs together), message
  prefixed with the task ID (e.g. `1.1: add retry logic to webhook handler`)
  so history stays traceable and any single task can be reverted on its own.
**Docs affected:** <exact path(s) from the Documentation Map — e.g.
  ...
  isn't "none" — create it. Never leave this field blank. Written only
  after Tier 1 is clean for this task.>
...
This is the core discipline of the protocol: verification happens in two tiers, at two different points in time, and docs are never written ahead of the tier that proves the code actually works. Getting the order right here is what stops a clean-looking `TASKS.md` from hiding a project that doesn't build.

| | Tier 1 — Per-task gate | Tier 2 — Mission gate |
|---|---|---|
| **Scope** | Build, lint, and tests touching this task's changed files | The whole project: full build, full lint, the entire existing test suite |
| **Runs** | Once per task, immediately after implementing it — before that task's docs are written or its commit is made | Once, after every task in the mission is already committed and documented |
| **Purpose** | Catch the bug where it was introduced, before any doc debt accumulates on top of it | Catch cross-task or integration regressions Tier 1 couldn't see — a safety net, not the primary place bugs are expected to surface |
| **On failure** | Fix and re-run Tier 1 for this task. Its docs stay unwritten and it stays uncommitted until this is clean. | Treat it as drift — see [Handling drift](#handling-drift) below. Fix, re-run Tier 2, land the fix as its own commit tagged to the original task ID, and reopen that task's line in `TASKS.md`. |
| **Applies even when...** | The task touches no code — build/test have nothing to run, but linting on whatever did change (docs, config) still applies | The mission touches no code at all (a pure doc bootstrap or doc-only edit) — Tier 2 still runs in full, as cheap regression insurance that nothing was silently broken |

#### Tier 1 — what to check, per task

Apply whichever of these are relevant to the specific task at hand:
...
Only once build, lint, and the relevant tests are all clean does the task's "Docs affected" field get written, followed by the single atomic commit and the `TASKS.md` checkbox.

#### Tier 2 — what to check, once, at the end of the mission

- Full build of the project — not just the packages touched.
- Full linter run — not just the changed files.
- Full existing test suite — not just the tests relevant to individual tasks; this is what catches interactions Tier 1's narrower scope couldn't.
- Documentation re-sync — every "Docs affected" path across the whole plan still matches current behavior. Don't just trust each task's Tier 1-era doc edit; a fix made during Tier 2 can invalidate a doc that was already correct when it was written. If a task was marked "none," confirm that's still true.

Fix whatever any of the four surfaces turns up, then repeat until all four are clean. Nothing is verified until it builds, lints, passes the full suite, and its docs match reality — at mission scope, not just task scope.
...
This is also the recovery procedure when Tier 2 fails. A Tier 2 failure means something that looked settled at Tier 1 wasn't — that's drift, not a fresh bug to patch quietly:

1. Fix the code.
2. Re-run Tier 2 in full until clean.
3. Land the fix as its own commit, tagged to the originating task ID — e.g. `1.3-fix: correct null-handling exposed by integration gate`. Never amend or rebase into the original task's commit; that erases the record of what Tier 1 actually verified at the time, and the fix-commit is the traceable evidence that Tier 2 did its job.
4. Reopen that task's line in `TASKS.md` (uncheck it, note the reason), and re-check it only once the fix, its own Tier 1 pass, and any doc correction have all landed.

If the fix invalidates a doc that Tier 1 already wrote, correct that doc in the same fix commit.

---

## Step 4 — Task Checklist → TASKS.md

Mirror every task from `PLAN.md` as a checkbox, same order, same names, including its docs field. This is the single source of truth for "is this done" — check an item off immediately after its Tier 1 gate passes, its commit is made, and its listed docs are updated, not before and not in a batch at the end. This is what stops an agent picking the work up cold from re-doing or duplicating something already finished: read `TASKS.md` first, touch only unchecked items.

The mission-level Tier 2 gate is its own separate checklist block at the bottom of `TASKS.md`, and it only gets touched once every task above it is already checked off — never in parallel, never early.
...
## Milestone 1: <name>
- [ ] 1.1 <task name> — tier 1: <pass/fail> — commit: <hash/message> — docs: <path(s) or "n/a">
- [ ] 1.2 <task name> — tier 1: <pass/fail> — commit: <hash/message> — docs: <path(s) or "n/a">

## Mission Verification (Tier 2) — only after every task above is checked
- [ ] Full build clean
- [ ] Full lint clean
- [ ] Full existing test suite passes
- [ ] Documentation re-sync confirmed (every "docs" field above still matches reality)

<!-- If Tier 2 fails: uncheck the affected task line(s) above, note the fix-commit
     reference next to it (e.g. "1.3 — reopened, see 1.3-fix"), and don't
     re-check the Tier 2 boxes until the fix's own Tier 1 pass is clean. -->
```
...
Only once Tier 2 is clean, the checklist is fully checked, and the sync above is resolved. Deliver this directly to the user — chat, PR description, or commit message — rather than a new persistent file, unless the project keeps a running `CHANGELOG.md`.

Cover, briefly:

- **What was done** — task by task, plain language, with the commit each one landed in.
- **Verification** — confirm Tier 1 passed for every task and Tier 2 passed once at mission scope; call out any fix-commits Tier 2 required and what they addressed.
...
- **Sync status** — state plainly whether it pushed, or is still pending and why (diverged, no remote configured, etc.), per the Sync to remote step above.
...
- Every non-trivial request goes through all five steps, in order. Don't skip Context Building because a request "looks simple" — that's exactly how duplicate or contradictory fixes happen. Trivial, single-file, low-risk requests may use the Step 0 fast path instead.
- Docs are never written for a task before that task's own Tier 1 gate is clean. The mission's Tier 2 gate is a regression safety net that always runs — including for zero-code, doc-only missions — but it is never the first place a bug is expected to be caught.
- A Tier 2 failure is fixed with its own commit tagged to the originating task ID, never by amending or rebasing an already-committed task.
- Never check off a `TASKS.md` item without its Tier 1 gate passing, its commit made, and its docs field resolved. Never check off the Tier 2 block until every task above it already is.
...
- A mission isn't shipped until it's pushed, and never pushed blind: see Step 4's [Sync to remote](#sync-to-remote) for the branch/remote-match/divergence check that must pass first, run the moment the checklist — tasks and Tier 2 alike — is fully checked off.
- When a request ships, archive it below and clear the Request block so the next session starts clean.

### Pipeline at a Glance

```mermaid
flowchart TD
    A[Implement Task N] --> B{Tier 1 gate:<br/>build + lint + relevant tests}
    B -- fail --> A
    B -- pass --> C[Write/update Task N's docs]
    C --> D[Commit code + docs together]
    D --> E[Check off TASKS.md]
    E -- more tasks remain --> A
    E -- all tasks done --> F{Tier 2 gate:<br/>full build + full lint + full suite<br/>+ doc re-sync}
    F -- fail --> G[Handle as drift:<br/>fix, tagged fix-commit,<br/>reopen the affected task line]
    G --> F
    F -- pass --> H[Sync to remote]
    H --> I[Walkthrough]
```
...
Runs once per doc set, whenever Step 2 finds that `docs/internal-devsguide/` and/or `docs/public-userguide/` don't exist yet — whether the codebase is large and undocumented, or this is a fresh project with barely any code. Treat it as Milestone 1 of a normal `PLAN.md`/`TASKS.md`; Steps 3–5 still apply, this just replaces a narrow code read with a wider first pass. Since discovery, outlining, and self-review touch no code, their "Tier 1 gate" reduces to whatever linting applies to the files actually changed (markdown lint, link-checking) — there's simply nothing to build or test yet, not an excuse to skip the check. Tier 2 still runs in full at the end, per the Operating Rules above.
...
2. **Outline** — propose headings-only outlines for both doc sets, using the Documentation Map as the file list, plus your open questions. Stop and get the user's go-ahead before writing content — this gate is separate from `PLAN.md` sign-off.
3. **Populate, staged** — write one internal page and one user-facing page first (default: `00-mission-and-overview.md` and `00-getting-started.md`, or whichever two the user cares most about), then pause and show the user those two before continuing. This catches a wrong tone or depth early instead of after all the files are written. Once confirmed, populate the rest per the Documentation Map and the Conventions below. Files under `docs/public-userguide/04-legal-and-policies/` get a factual inventory only (never invented boilerplate) plus the DRAFT marker.
